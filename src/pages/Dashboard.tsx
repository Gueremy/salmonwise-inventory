  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <KpiCard label="Ocupación Global" value="67%" sub="+3% vs semana pasada" icon={Box} accent="bg-primary/10 text-primary" />
        <KpiCard label="Alertas Activas" value="3" sub="1 crítica" icon={Bell} accent="bg-destructive/10 text-destructive" />
        <KpiCard label="Movimientos Hoy" value="12" sub="3 pendientes" icon={ArrowUpRight} accent="bg-status-disponible/10 text-status-disponible" />
        <KpiCard label="Próx. Vencimiento" value="4 días" sub="LOT-2026-03-089" icon={Clock} accent="bg-status-medio/10 text-status-medio" />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-card rounded-lg border border-border/60 p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-semibold">Ocupación por galpón</h3>
              <p className="text-xs text-muted-foreground">Planta Puerto Montt</p>
            </div>
          </div>
          <div className="h-72">
            <ResponsiveContainer>
        ...
        <div className="bg-card rounded-lg border border-border/60 p-5 shadow-sm">
          ...
          <div className="space-y-3">
            {[
              { sev: "critica", txt: "G2-C08 al 95% de capacidad", time: "hace 1h", color: "bg-destructive" },
              { sev: "critica", txt: "LOT-2026-03-089 vence en 3 días", time: "hace 2h", color: "bg-destructive" },
              { sev: "warn", txt: "G3-C02 sin revisión > 48h", time: "hace 5h", color: "bg-status-medio" },
              { sev: "info", txt: "Sincronización offline OK", time: "hace 8h", color: "bg-secondary" },
            ].map((a, i) => (
      ...
      <div className="bg-card rounded-lg border border-border/60 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-border">
          <h3 className="font-semibold">Movimientos pendientes de aprobación</h3>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-xs uppercase text-muted-foreground">
              <tr>
                ...
                <th className="text-left px-5 py-3 font-medium">Galpón</th>
            ...
            <tbody>
              {pendientes.length === 0 && (
                <tr><td colSpan={7} className="text-center py-10 text-muted-foreground">Sin movimientos pendientes 🎉</td></tr>
              )}
              {pendientes.map((m, i) => (
                <tr key={m.id} className="border-t border-border hover:bg-muted/30 transition">
                  ...
                  <td className="px-5 py-3 text-right space-x-2">
                    <Button size="sm" variant="outline" className="border-status-disponible/40 text-status-disponible hover:bg-status-disponible/10" onClick={() => aprobar(m.id)}>Aprobar</Button>
                    <Button size="sm" variant="outline" className="border-destructive/40 text-destructive hover:bg-destructive/10" onClick={() => setRejecting(m.id)}>Rechazar</Button>
                  </td>
                </tr>
              ))}
            </tbody>
